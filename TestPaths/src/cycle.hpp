...
struct Cycle {
  ...
  int K;
  bool an = false;
  Cycle(int _n, int _K, bool _an = false) : n(_n), K(_K), an(_an) {}
  ...
  void generate(std::string filename) {
    ...
    for (int i = 0; i < K; i++) {
      for (int j = 0; j < K; j++) {
        if (i != j) {
          file << -vertex_id(n, K, n - 1, i) << -vertex_id(n, K, 0, j)
               << hom_edge_id(n, K, i, j, 0) << End;
          file << -vertex_id(n, K, n - 1, i) << -vertex_id(n, K, 0, j)
               << -hom_edge_id(n, K, i, j, 1) << edge_id(n - 1) << End;
          file << -vertex_id(n, K, n - 1, i) << -vertex_id(n, K, 0, j)
               << hom_edge_id(n, K, i, j, 1) << -edge_id(n - 1) << End;
        }
      }
    ...
    for (int i = 0; i < K; i++) {
      for (int j = 0; j < K; j++) {
        if (i != j) {
          file << -X_id(n, K, n - 1, i, j, 1) << vertex_id(n, K, n - 1, i)
               << End;
          file << -X_id(n, K, n - 1, i, j, 0) << vertex_id(n, K, n - 1, i)
               << End;
          file << -X_id(n, K, n - 1, i, j, 1) << vertex_id(n, K, 0, j) << End;
          file << -X_id(n, K, n - 1, i, j, 0) << vertex_id(n, K, 0, j) << End;
          file << -X_id(n, K, n - 1, i, j, 1) << edge_id(n - 1) << End;
          file << -X_id(n, K, n - 1, i, j, 0) << -edge_id(n - 1) << End;

          file << X_id(n, K, n - 1, i, j, 1) << -vertex_id(n, K, n - 1, i)
               << -vertex_id(n, K, 0, j) << -edge_id(n - 1) << End;
          file << X_id(n, K, n - 1, i, j, 0) << -vertex_id(n, K, n - 1, i)
               << -vertex_id(n, K, 0, j) << edge_id(n - 1) << End;
        }
      }
    ...
    for (int i = 0; i < K; i++) {
      file << -vertex_id(n, K, n - 1, i) << -vertex_id(n, K, 0, i) << End;
    }
